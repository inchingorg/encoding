...
program.command('base64 <data>')
    .description('Convert .')
    .alias('b')
    .option('-f, --from [source encoding]', 'Source encoding, default utf-8', "utf-8")
    .option('-t, --to [source encoding]', 'Destination encoding, default utf-8', "utf-8")
    .option('-f, --format [format]', 'Output format style, options are hex|compress|map|array, hex is default.', "/^(hex|compress|map)", 'hex')
    .action(function (data) {
        var result = convert(data, this.from, this.to, this.format);
        console.log(result);
    });

function convert(data, from ,to, format){
    if (from === to) {
        encode(from, to, );
    ...
    if (to === 'UTF-8') {
        ...
        return iconv.encode(data, to);
    } else {
        return iconv.encode(iconvLite.decode(data, from), to);
    }
}
...
function decode(data, encoding) {
    ...
    if (encoding.toLowerCase() === 'base64') {
        console.log('Please use convert command.')
    }
