...
describe('Decode', function () {
    ...
    it('decode pretty hex', function () {
        program.parse(['node', 'decode', 'd6 d0 b9 fa', '--encoding', 'gbk']);
        console.log.should.be.calledWith('中国');
    });

    it('decode compressed hex', function () {
        program.parse(['node', 'decode', 'd6d0b9fa', '--encoding', 'gbk']);
        console.log.should.be.calledWith('中国');
    });

    it('decode array hex', function () {
        program.parse(['node', 'decode', '[0xd6, 0xd0, 0xb9, 0xfa]', '--encoding', 'gbk']);
        console.log.should.be.calledWith('中国');
    });
